...
const LanguageSelectionView: React.FC<LanguageSelectionViewProps> = ({ onSelect, onBack }) => {
  const [showAll, setShowAll] = useState(false);
  const [pendingLanguage, setPendingLanguage] = useState<string | null>(null);
  
  const topLanguages = LANGUAGES
    .filter(l => TOP_LANG_CODES.includes(l.code))
    .sort((a, b) => TOP_LANG_CODES.indexOf(a.code) - TOP_LANG_CODES.indexOf(b.code));
  ...
  return (
    ...
    >
      ...
      <div className="text-center pt-16 pb-6 px-6 shrink-0 bg-slate-50 z-10">
        ...
        <h1 className="text-2xl font-black text-gray-900 tracking-tight">NearbyKaam</h1>
        <p className="text-gray-500 font-medium mt-1">Select Language • भाषा चुनें</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-20 no-scrollbar relative">
        ...
        <div className="grid grid-cols-2 gap-4">
          {topLanguages.map((lang) => (
            <button
              ...
              className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center active:bg-orange-50 hover:border-orange-500 hover:shadow-md hover:scale-[1.02] transition-all duration-200 group"
            >
              <span className="text-lg font-bold text-gray-900 mb-1 group-hover:text-orange-600">{lang.nativeName}</span>
              <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest group-hover:text-orange-400">{lang.name}</span>
            </button>
          ))}
          
          {/* Expanded Languages render inside the same grid */}
          {showAll && otherLanguages.map((lang) => (
            <button
              ...
              className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center active:bg-orange-50 hover:border-orange-500 hover:shadow-md hover:scale-[1.02] transition-all duration-200 group animate-in fade-in slide-in-from-bottom-2"
            >
              <span className="text-lg font-bold text-gray-900 mb-1 group-hover:text-orange-600">{lang.nativeName}</span>
              <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest group-hover:text-orange-400">{lang.name}</span>
            </button>
        ...
        >
          {showAll ? 'Show fewer languages' : 'View More Languages'} <i className={`fa-solid ${showAll ? 'fa-chevron-up' : 'fa-chevron-down'} ml-2`}></i>
        </button>
      </div>
      ...
      {pendingLanguage && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-xs rounded-[32px] p-8 shadow-2xl text-center animate-in zoom-in-95 duration-300">
            ...
            <h2 className="text-xl font-black text-slate-900 mb-3">Change language?</h2>
